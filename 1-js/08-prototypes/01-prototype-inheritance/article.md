...
عند قراءة خاصية من `كائن` وتعذر وجودها، يقوم JavaScript تلقائيًا بأخذها من النموذج الأصلي. في البرمجة، يُطلق على مثل هذا الأمر "الوراثة النمطية". وقريبًا سندرس العديد من الأمثلة على هذه الوراثة، بالإضافة إلى ميزات لغوية أكثر إثارة تستند عليها.
...
الآن إذا قرأنا خاصية من `rabbit`، ولم تكن موجودة، سيقوم JavaScript تلقائيًا بأخذها من `animal`.
```js
let animal = {
  eats: true
};
...
الآن إذا قرأنا شيئًا من `longEar`، ولم يتم العثور عليه، سيبحث JavaScript عنه أولاً في `rabbit`، ثم في `animal`.
